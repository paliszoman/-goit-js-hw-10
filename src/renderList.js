...
export let renderCountries = (countries) => {
  const markup = countries
    .map((country) => {
      ...
      <span class="country-span" style="padding-left: 5px; font-weight: 500; font-size: 18px">${country.name.official}</span>
      </li>`;
    })
    .join('');
  ...
  if (countries.length < 10 && countries.length > 2) {return countryList.innerHTML = "",
...
let mainCountry = (countries) => {
     countries
      ...
      <li style="list-style:none;"><span style="padding-right: 41px; font-weight: 700;">Capital: </span>${country.capital}</li>
